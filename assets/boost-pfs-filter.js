...
(function() {
  ...
  ProductGridItem.prototype.compileTemplate = function(data, index) {
    ...
    var itemTypeHtml = boostPFSConfig.custom.type_enable ? '<div class="grid-product__vendor">' + data.product_type + '</div>' : '';
    itemHtml = itemHtml.replace(/{{itemType}}/g, itemTypeHtml);
    // Add Quick shop
    ...
    if (boostPFSConfig.custom.quick_shop_enable) {
      ...
      itemQuickShopButtonHtml = '<div class="quick-product__btn js-modal-open-quick-modal-{{itemId}} small--hide ' + (isFirstLoad ? '' : 'bc-hide') + '" data-product-id="{{itemId}}">' +
        ...
        '<button class="quick-product__label">' + boostPFSConfig.label.quick_shop + '</button>' +
        '</div>';
    }
    ...
    var reviewHtml = '<div class="pwr-category-snippets" style=""><div id="snippet-'+data.id+'"></div></div>';
    ...
    var BannerHtml = boostPFSTemplate.productBannerItem;
    var BannerPosition = boostPFSTemplate.productBannerPosition;
    console.log(index)
    if (index == BannerPosition - 2) {
      itemHtml += BannerHtml
    }
    return itemHtml;
  };
        ...
        if (option_name.indexOf('color') != -1 || option_name.indexOf('colour') != -1) {
          ...
          data.variants.forEach(function(variant) {
            ...
            if (values.indexOf(value) == -1) {
              ...
              values.push(value);
              var colorValueSlugify = 'color-'+ Utils.slugify(value);
              console.log(colorValueSlugify)
              var colorImage = Utils.optimizeImage(boostPFSAppConfig.general.swatch_file_url.replace('swatch', colorValueSlugify + '.png'), '50x');
  ...
  Filter.prototype.afterRender = function(data, eventType) {
    ...
    var bold_hidden_product_cnt=0;
    for (let i = 0; i < data.products.length; i++) {
      const product = data.products[i];
      if (product.tags.includes("BOLD_HIDDEN_PRODUCT")){
        bold_hidden_product_cnt++;
      }
    }
    ...
    if (data.total_product == 1) {
      ...
      totalProductText = boostPFSConfig.label.items_with_count_other.replace(/{{ count }}/g, data.total_product - bold_hidden_product_cnt);
    }
